...
export default function QRGenerator() {
  ...
  const generateQR = async () => {
    ...
    const qrData = await QRCode.toDataURL(text, {
      ...
      margin: 2,
      color: {
        dark: "#1e293b", // ปรับสีตัว QR ให้เป็นสีน้ำเงินเข้มละมุน ไม่ดำสนิท
        light: "#ffffff",
      },
    });
  ...
  return (
    // ส่วนพื้นหลังไล่ระดับสี และจัดตำแหน่งให้อยู่ตรงกลางหน้าจอ
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-indigo-50 via-white to-cyan-50 p-6">
      ...
      <div className="w-full max-w-md bg-white/80 backdrop-blur-lg shadow-xl shadow-indigo-100/40 rounded-3xl p-8 border border-indigo-100/80 flex flex-col gap-6 transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-100/60">
        ...
        {/* ส่วนอินพุตและปุ่ม (Input & Button) */}
        ...
        {/* ส่วนแสดงผล QR Code (Result) */}
        {qr && (
          <div className="mt-2 pt-6 border-t border-slate-100 flex flex-col items-center justify-center animate-fade-in">
            {/* กรอบของ QR Code */}
            <div className="bg-slate-50 p-4 rounded-3xl border border-slate-100 shadow-inner">
              <img
                src={qr}
                alt="QR Code"
                className="w-56 h-56 rounded-lg transition-transform duration-300 group-hover:scale-105"
              />
            </div>
            {/* คำแนะนำ */}
